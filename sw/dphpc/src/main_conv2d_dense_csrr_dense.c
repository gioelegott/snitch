...
#include "snrt.h"
#include "printf.h"
#include "data_conv2d_dense_csrr_dense.h"
...
csrr_matrix *matrix_FILTER[CHANNELS][CHANNELS];
  ...
  if (snrt_is_dm_core()) {
    for (int j = 0; j < CHANNELS; j++) {
      ...
      for (int k = 0; k < CHANNELS; k++) {
        ...
        matrix_FILTER[k][j]->row_idx = snrt_l1alloc(FILTER[k][j].nnz  * sizeof(int));
      ...
      for (int k = 0; k < CHANNELS; k++) {
        ...
        snrt_dma_start_1d((void *)matrix_FILTER[k][j]->row_idx, (void *)FILTER[k][j].row_idx, FILTER[k][j].nnz  * sizeof(int));
  ...
  // Serial
    ...
    if (compute_id == 0) {  
      ...
      for (int i = 0; i < CHANNELS; i++) { 
        conv2d_dense_csrr_dense(matrix_A, matrix_FILTER[i], matrix_res[i], CHANNELS, A_col, res_row, res_col);
      }
      benchmark_get_cycle();
    }
