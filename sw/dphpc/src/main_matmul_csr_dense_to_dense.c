...
dense_matrix *matrix_B, *matrix_res_dense;
...
int main() {
  ...
  if (NUM_COMP_CORES == 1) {
    benchmark_get_cycle();
    matmul_csr_dense_to_dense(matrix_A, matrix_B, matrix_res_dense);
    benchmark_get_cycle();
  } else {
    benchmark_get_cycle();
    ...
    matmul_csr_dense_to_dense(&matrix_A_parallel, matrix_B, &matrix_res_parallel);
    benchmark_get_cycle();
  }
...
#ifndef MEASUREMENT
  if (compute_id == 0) {
    for (int i = 0; i < C_dense.rows * C_dense.cols; i++) {

      if (fabs(matrix_res_dense->values[i] - C_dense.values[i]) > 0.001) {
        errors++;
      }
    }

    if (errors != 0) {
      printf("Errors: %d/%d!\n", errors, C_dense.rows * C_dense.cols);
    }
  }
#endif
  return errors;
