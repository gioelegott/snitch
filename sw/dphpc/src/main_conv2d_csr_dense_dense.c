...
#include "snrt.h"
#include "printf.h"
#include "data_conv2d_csr_dense_dense.h"
...
csr_matrix *matrix_A[CHANNELS];  
dense_matrix *matrix_FILTER[CHANNELS][CHANNELS], *matrix_res[CHANNELS]; 
  ...
  // Serial
    ...
    if (compute_id == 0) {  
      ...
      for (int i = 0; i < CHANNELS; i++) { 
        conv2d_csr_dense_dense(matrix_A, matrix_FILTER[i], matrix_res[i], CHANNELS, filter_row, filter_col, res_row, res_col);
      }
      benchmark_get_cycle();
    }
